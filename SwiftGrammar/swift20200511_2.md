...
// Struct는 값 타입이기 때문에 Stack에 있는 값을 let으로 만들면 변경되지 않음
//cat.name = "토리" // error
...
/*
 var로 선언된 변수(프로퍼티)
 */
// Struct는 프로퍼티 초기화를 시켜서 저장을 해도 인스턴스화할 때 변경할 수 있게 제공을 해줌
// class에서 위와 같이 하려면 따로 init메서드를 작성해줘야함 init(name: String) { self.name = name }
// 기존에 만들었던 초기화 값을 사용하려면 또 별도로 init() { self.name = "홍길동" }으로 작성을 해야함
...
struct PointStruct {
  ...
  // 값 타입은 값을 변경하려면 명시적으로 mutating을 작성해 줘야함
// mutating func updateX() {
//    self.x = 5
//  }

 // 연산 프로퍼티에서는 기본적으로 mutating이므로 작성하면 error가 남
 // 함수에서만 작성 함
