...
final class TableViewSection: UIViewController {
  ...
  private func dictSet() {
    var counter = 10
    var keyNum = 0
    for i in data {

      if i < counter {
        if numberCategories.keys.contains(keyNum) {
          numberCategories[keyNum]?.append(i)
        } else {
          keyNum = i
          numberCategories[keyNum] = [i]
        }
      } else {
        counter += 10
        keyNum = i
...
extension TableViewSection: UITableViewDataSource {
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    ...
    let number = numberCategories[sectionTitles[indexPath.section]]!
    print(numberCategories[sectionTitles[indexPath.section]]!)
